...
// 第一人称「此刻」在场感：让 AI 以「我就是角色、此刻正在这里」的方式进入画面（方法派）
function firstPersonPresence(character) {
  if (!state.scene) advanceSceneState();
  const sc = state.scene;
  const isMarisa = character === 'marisa';
  const selfName = isMarisa ? '魔理沙' : '灵梦';
  const otherName = isMarisa ? '灵梦' : '魔理沙';
  const selfAction = isMarisa ? sc.marisaAction : sc.reimuAction;
  const otherAction = isMarisa ? sc.reimuAction : sc.marisaAction;
  const note = sc.note ? '，' + sc.note : '';
  return (
    `此刻你（${selfName}）正${selfAction}；${otherName}正${otherAction}。` +
    `你${sc.mood}${note}。` +
    `这些就发生在你眼前、你身边，是你正在经历的一刻——让对话从这片具体的日常里长出来。`
  );
}
...
module.exports = { advanceSceneState, sceneStateText, displaySceneText, firstPersonPresence, updateSceneNote };
