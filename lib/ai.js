...
function buildMessages({ character, topic, history = [], persona, summary, canon }) {
  ...
  messages.push({
    ...
    content:
      ...
      `当前时间：现在是幻想乡的${timePeriodLabel()}（按真实世界时间，可以自然地呼应天色与作息，但不要报出具体钟点和现实日期）。\n` +
      `此刻画面：${displaySceneText()}\n` +
      `天气氛围：${weatherInjectionText()}\n` +
      ...
      `关系温度：两人亲密到彼此心照不宣（相当于好感度体系的「思慕」——都把彼此当成最重要的人，深深在意、愿意为对方付出，会分享心事、主动邀请对方、约好明天见）。日常的斗嘴和嫌弃是亲密的表现，不是真的冷淡；甜要落在细节里：留的茶、留的饭、记得的喜好、第一个想分享的人。`
  });
