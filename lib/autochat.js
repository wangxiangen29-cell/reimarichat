...
const { advanceSceneState } = require('./scene');
const { maybeAmbient, ambientLine } = require('./ambient');
const { pickRandomTopic, rotateTopic } = require('./topics');
const { generateAutoBatch, generateAutoMessage, aiUsable, speakerName, callAI, stripSelfNamePrefix } = require('./ai');
...
async function agentChatTick() {
  ...
  try {
    // 每轮推进「此刻画面」，让第一人称在场感随场景流转，不一直定格在同一动作
    advanceSceneState();
    const text = await generateAutoMessage(speaker);
    if (!text) return;
    state.consecutiveErrors = 0;
    const cleaned = stripSelfNamePrefix(text, speaker) || text;
    pushLog(speaker, cleaned);
    state.aiHistory.push({ speaker, text: cleaned });
    if (state.aiHistory.length > AI_HISTORY_MAX) state.aiHistory.shift();
    maybeAmbient();
  } catch (err) {
...
async function autoChatTick() {
  ...
  pushLog(speaker, text, item.emotion);
  state.aiHistory.push({ speaker, text });
  if (state.aiHistory.length > AI_HISTORY_MAX) state.aiHistory.shift();
  maybeAmbient();
}
