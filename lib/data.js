...
// ---------- Seed / 迁移 ----------
// 版本检测：数据文件 version 落后于 DATA_VERSION 时，先备份旧文件（backup-<name>-v<old>-<ts>.json），
// 再重新 seed 新版默认。personas.js 是提示词/知识库的单一来源，不再优先取 config 的 legacy 字段。
const DATA_VERSION = 2;

function backupJsonFile(file, oldVersion) {
  const ts = new Date().toISOString().replace(/[:.]/g, '-');
  const backupPath = path.join(DATA_DIR, `backup-${path.basename(file, '.json')}-v${oldVersion}-${ts}.json`);
  try {
    fs.copyFileSync(file, backupPath);
    console.log(`已备份旧版 ${path.basename(file)} → ${path.basename(backupPath)}`);
  } catch (err) {
    console.error(`备份 ${path.basename(file)} 失败：`, err.message);
  }
}
...
function buildDefaultAgents() {
  ...
  for (const ch of ['reimu', 'marisa']) {
    ...
    agents[ch] = {
      base_prompt: ch === 'reimu' ? DEFAULT_REIMU_PERSONA : DEFAULT_MARISA_PERSONA,
      goals: d.goals,
      memory_instructions: d.memory_instructions,
      few_shots: d.few_shots
    };
  }
  return { version: DATA_VERSION, agents };
}

function seedAgents() {
  if (fs.existsSync(AGENTS_FILE)) {
    agentsStore = loadJson(AGENTS_FILE, { version: DATA_VERSION, agents: {} });
    const v = (agentsStore && agentsStore.version) || 1;
    if (v < DATA_VERSION) {
      backupJsonFile(AGENTS_FILE, v);
      agentsStore = buildDefaultAgents();
      atomicWriteJson(AGENTS_FILE, agentsStore);
      console.log('agents.json 已迁移到 v' + DATA_VERSION);
    }
    return;
  }
  agentsStore = buildDefaultAgents();
...
function buildDefaultCanon() {
  const entries = [
    { id: 'reimu-yishe-1', character: 'reimu', category: '一设', content: DEFAULT_CANON_REIMU, sort: 1 },
    { id: 'marisa-yishe-1', character: 'marisa', category: '一设', content: DEFAULT_CANON_MARISA, sort: 1 },
    { id: 'shared-shijieguan-1', character: 'shared', category: '世界观', content: DEFAULT_CANON_WORLD, sort: 1 },
    { id: 'shared-guanxi-1', character: 'shared', category: '关系', content: DEFAULT_CANON_PAIR, sort: 1 },
    { id: 'shared-yicuotixing-1', character: 'shared', category: '易错提醒', content: DEFAULT_CANON_NOTES, sort: 1 }
  ];
...
function seedCanon() {
  if (fs.existsSync(CANON_FILE)) {
    canonStore = loadJson(CANON_FILE, { version: DATA_VERSION, entries: [] });
    const v = (canonStore && canonStore.version) || 1;
    if (v < DATA_VERSION) {
      backupJsonFile(CANON_FILE, v);
      canonStore = buildDefaultCanon();
      atomicWriteJson(CANON_FILE, canonStore);
      console.log('canon.json 已迁移到 v' + DATA_VERSION);
    }
    return;
  }
  canonStore = buildDefaultCanon();
