...
function formatAiError(err) {
  ...
  if (/fetch failed|ECONNRESET|ETIMEDOUT|ENOTFOUND|ECONNREFUSED|EACCES|socket hang up/i.test(message)) {
    return `AI 网络连接失败${causeCode}：无法连接到当前接口地址，请检查网络、代理或设置里的接口地址`;
  }
  return message;
}
...
async function handleChat(req, res, body) {
  ...
  try {
    ...
    sendJSON(req, res, 502, { error: `调用 AI 失败：${formatAiError(err)}` });
  }
}
...
async function handleChatBatch(req, res, body) {
  ...
  try {
    ...
    messages.push({
      role: 'user',
      content: `请以 JSON 对象格式输出，格式必须严格为 {"lines":[{"text":"台词一","emotion":"smile"},{"text":"台词二","emotion":"normal"}]}，其中 lines 必须恰好包含 ${count} 个元素，一条都不能少（先在心中列好 1 到 ${count} 的台词清单，再逐条完整输出；宁可每句短一点，也要把 ${count} 条写满；每句 1~3 句中文，口语自然，符合角色性格），第一句由${speakerName(character)}说，之后两人严格交替。emotion 是这句台词说话人的立绘表情，相当于一次界面调用（switch_portrait），只能从这些里选一个：${emotionMenuText()}，按台词内容认真选择，不明确就用 normal。每句都要有新内容，接住对方上一句的具体细节，不要重复上文中已经说过的话；要有甜甜的小剧情和心动细节，让情绪有来有回，不要公式化一问一答。只输出这个 JSON 对象，不要角色名前缀、不要任何解释；每句台词都直接以说话内容开头，严禁在台词开头写出说话人自己的名字（比如灵梦说的话绝不能以「灵梦」二字开头，魔理沙也不能以「魔理沙」开头）。`
    });
    ...
    const deduped = dedupeLines(items.map((it) => it.text), (history || []).slice(-10).map((m) => m.text));
    const emoByText = new Map(items.map((it) => [it.text, it.emotion]));
    updateSceneNote(deduped);
    let sp = character;
    const replies = deduped.map((text) => {
...
// ---------- 油库里语音代理：浏览器统一走 /api/tts，由服务器转发到本机 yukkuri 服务 ----------
async function handleTts(req, res, searchParams) {
  const text = String(searchParams.get('text') || '').slice(0, 300).trim();
  const speaker = searchParams.get('speaker') === 'marisa' ? 'marisa' : 'reimu';
  if (!text) return sendJSON(req, res, 400, { error: '缺少 text 参数' });
  const base = String(config.voiceTtsUrl || 'http://127.0.0.1:9120').replace(/\/+$/, '');
  try {
    const upstream = await fetch(`${base}/tts?text=${encodeURIComponent(text)}&speaker=${speaker}`, {
    ...
    sendJSON(req, res, 502, { error: '油库里语音服务不可用：请在服务器上运行 python tools/yukkuri_server.py' });
  }
}

// ---------- 情绪注册表（标注页可增删，AI 协议与立绘切换共用） ----------
const { loadEmotions, saveEmotions, normalizeEmotion, emotionHasArt, emotionMenuText, emotionKeys } = require('./emotions');

function handleEmotionsGet(req, res) {
  sendJSON(req, res, 200, { emotions: loadEmotions() });
}

function handleEmotionsWrite(req, res, body) {
  const { action, key, label, keywords } = body || {};
  const list = loadEmotions();
  if (action === 'add') {
    const { item, error } = normalizeEmotion({ key, label, keywords });
    if (error) return sendJSON(req, res, 400, { error });
    if (list.some((e) => e.key === item.key)) return sendJSON(req, res, 400, { error: '这个情绪 key 已存在' });
    if (list.length >= 24) return sendJSON(req, res, 400, { error: '情绪种类最多 24 个' });
    list.push(item);
  } else if (action === 'remove') {
    const k = String(key || '').trim();
    const idx = list.findIndex((e) => e.key === k);
    if (idx === -1) return sendJSON(req, res, 404, { error: '情绪不存在' });
    list.splice(idx, 1);
  } else {
    return sendJSON(req, res, 400, { error: '未知操作（add / remove）' });
  }
  saveEmotions(list);
  sendJSON(req, res, 200, { ok: true, emotions: list });
}

// ---------- 立绘差分标注：标注页把压缩后的 PNG 存成正式差分文件 ----------
const fs = require('fs');
const path = require('path');

function handlePortraitSave(req, res, body) {
  const file = String((body && body.file) || '');
  const dataUrl = String((body && body.dataUrl) || '');
  const m = file.match(/^(reimu|marisa)_([a-z0-9_]+)$/);
  if (!m) return sendJSON(req, res, 400, { error: '非法的立绘文件名' });
  const emoKey = m[2] === 'full' ? 'normal' : m[2];
  if (!emotionKeys().has(emoKey)) return sendJSON(req, res, 400, { error: '未注册的情绪：' + emoKey });
  const dm = dataUrl.match(/^data:image\/png;base64,([A-Za-z0-9+/=]+)$/);
  if (!dm) return sendJSON(req, res, 400, { error: '需要 PNG 格式的 dataURL' });
  const buf = Buffer.from(dm[1], 'base64');
  if (buf.length > 4 * 1024 * 1024) return sendJSON(req, res, 400, { error: '图片过大（>4MB）' });
  if (buf[0] !== 0x89 || buf.slice(1, 4).toString() !== 'PNG') {
    return sendJSON(req, res, 400, { error: '内容不是 PNG 图片' });
  }
  try {
    fs.writeFileSync(path.join(__dirname, '..', 'public', 'avatars', file + '.png'), buf);
  } catch (err) {
    return sendJSON(req, res, 500, { error: '写入失败：' + err.message });
  }
  sendJSON(req, res, 200, { ok: true, file: file + '.png' });
}
